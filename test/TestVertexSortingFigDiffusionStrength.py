...
data_dir = os.path.join(os.environ['CHASTE_TEST_OUTPUT'], 'VertexIbComp', 'CellSorting', 'DiffStr')

list_of_sim_dirs = []

for _dir in os.listdir(data_dir):
    if os.path.isdir(os.path.join(data_dir, _dir)):
        list_of_sim_dirs.append(_dir)

print(list_of_sim_dirs)

for param_value in list_of_sim_dirs:

    sim_dir = os.path.join(data_dir, param_value)

    list_of_repeats = []
    for _dir in os.listdir(sim_dir):
        if os.path.isdir(os.path.join(sim_dir, _dir)):
            list_of_repeats.append(_dir)

    list_of_data_files = []

    for repeat in list_of_repeats:
        data_file = os.path.join(sim_dir, repeat, 'results_from_time_10', 'heterotypicboundary.dat')
        if os.path.isfile(data_file):
            list_of_data_files.append(data_file)

    print(list_of_data_files)

    list_of_hetero_lengths = []

    for data_file in list_of_data_files:
        a = np.genfromtxt(list_of_data_files[0], delimiter='\t')
        list_of_hetero_lengths.append(a[:, 1])

    print (sum(x for x in list_of_hetero_lengths) / len(list_of_hetero_lengths))
